...
def drawDFA(dfanode, dfaedge):
    dfa = Digraph('dfa_LALR(1)', format='png')
...
def condensenode(node):
    ...
    for i in node:
        ...
        if tmp in pro:
            idx = pro.index(tmp)
            res[idx] = (res[idx][0], res[idx][1], res[idx][2], tuple(set(res[idx][3]) | set(i[3])))
        else:
            res.append(i)
            pro.append(tmp)
    return tuple(res)
...
def andnode(node1, node2):
    res = []
    for i in range(len(node1)):
        res.append((node1[i][0], node1[i][1], node1[i][2], tuple(set(node1[i][3]) | set(node2[i][3]))))
    return res


def unionnode(dfaNode, dfaEdge):
    newNode = []
    proNode = []
    index = [-1] * len(dfaNode)
    for i in range(len(dfaNode)):
        tmp = getpro(dfaNode[i])    # 得到该节点的所有产生式
        if tmp in proNode:
            index[i] = proNode.index(tmp)
        else:
            index[i] = len(proNode)
            proNode.append(tmp)
    for i in range(len(dfaEdge)):
        dfaEdge[i] = (index[dfaEdge[i][0]], index[dfaEdge[i][1]], dfaEdge[i][2])
    for i in range(len(dfaNode)):
        tmp = dfaNode[i]
        if len(newNode) - 1 >= index[i]:
            newNode[index[i]] = andnode(newNode[index[i]], tmp)  # 将转换条件合并
        else:
            newNode.append(tmp)
    return newNode
...
def main():
    ...
    for i in range(len(dfaNode)):
        dfaNode[i] = condensenode(dfaNode[i])
    dfaNode = unionnode(dfaNode, dfaEdge)
    dfaEdge = set(dfaEdge)
    dfaEdge = list(dfaEdge)
    dfaEdge.sort(key=cmp)
